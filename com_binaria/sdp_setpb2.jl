...
function set_pb(m, t, r)
    ...
    y_min  = -50 * ones(nM, nY);  # output bounds
    y_max  =  50 * ones(nM, nY);  # output bounds

    u_min  =  1 * ones(nM, nU);  # control signal lower bound
    ...
    wm = 0.5 * ones(nM, nY, nT);  # reference trajectory

    r_max = r * ones(nR, nT - 1);
    rmu   = ones(nM, nR, nU);
