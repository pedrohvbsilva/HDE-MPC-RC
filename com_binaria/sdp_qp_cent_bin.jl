...
model = Model(optimizer_with_attributes(()->Gurobi.Optimizer(GRB_ENV), "Threads" => 1))

@variable(model, δ[1:nM[1], 1:nT[1] - 1], Bin);
...
# -----------------------
# QP OPtimization
@objective(model, Min, sum(rm[m] * Du[m,i,t]^2 for m in 1:nM[1], i in 1:nU[1], t in 1:Num[m] - 1) + 
...
# State Dynamic Equations
@constraint(model, x_eq_cons[m in 1:nM[1], i in 1:nX[1], t in 2:nT[1]], x[m,i,t] == dot(Am[m,i,:], x[m,:,t - 1]) + dot(Bm[m,i,:], u[m,:,t - 1]));

# Output Equations
@constraint(model, y_eq_cons[m in 1:nM[1], i in 1:nY[1], t in 2:nT[1]], y[m,i,t] == dot(Cm[m,i,:], x[m,:,t]) + dot(Dm[m,i,:], u[m,:,t - 1]));
...
@constraint(model, u_min_cons[m in 1:nM[1], i in 1:nU[1], t in 1:Num[m] - 1], (δ[m,t] * u_min[m,i]) - u[m,i,t] <= 0);
@constraint(model, u_max_cons[m in 1:nM[1], i in 1:nU[1], t in 1:Num[m] - 1], u[m,i,t] - (δ[m,t] * u_max[m,i]) <= 0);

# Resource constraints
@constraint(model, res_cons[m in 1:nM[1], r in 1:nR[1], t in 1:nT[1] - 1], sum(dot(rmu[m,r,:], u[m,:,t]) for m in 1:nM[1]) <= r_max[r,t]);
