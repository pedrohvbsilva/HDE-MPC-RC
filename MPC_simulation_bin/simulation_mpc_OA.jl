...
include("sdp_OA_master.jl")
include("sdp_OA_sub.jl")
include("sdp_OA_submf.jl")
include("sdp_OA_submi.jl")
...
#    @showprogress 1 "Computing..." for i in 2:T
    for i in 2:T
        @show i
        # Sm = sdp_OA_master(G);
        obj, Sm2 = sdp_OA_master(G);
         # global S;

        push!(Objetivo_OA, obj)
...
# matwrite("var_plot_OA.mat", Dict("u" => upl, "y"=>ypl, "obj_OA"=>Objetivo_OA))
...
writedlm("OAy.csv", ypl,',')
writedlm("OAsu.csv", upl,',')
writedlm("OAobj.csv", Objetivo_OA,',')
