...
include("sdp_benders_sub_bin.jl")
include("sdp_benders_submf_bin.jl")
include("sdp_benders_submi_bin.jl")
...
#    @showprogress 1 "Computing..." for i in 2:T
    for i in 2:T
        @show i
        # Sm = sdp_benders_master(G);
        obj, Sm2 = sdp_benders_master(G);
         # global S;

        push!(Objetivo_benders, obj)

        for m in 1:G.nM
            SS[m,i] = (du = Sm2[m].Du[1,1], u = Sm2[m].u[1,2], y = Sm2[m].y[1,2], x = Sm2[m].x[:,2]);
...
# matwrite("var_plot_benders.mat", Dict("u" => upl, "y"=>ypl, "obj_benders"=>Objetivo_benders))
...
savefig(fn, "m4benders.pdf")

writedlm("bendersy.csv", ypl,',')
writedlm("bendersu.csv", upl,',')
writedlm("bendersobj.csv", Objetivo_benders,',')
