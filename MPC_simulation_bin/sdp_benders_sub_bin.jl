...
function sdp_benders_sub_bin(ss, δ, G)
    ...
    # tempo1 = @elapsed sm_output = pmap(sdp_benders_submf_bin, ar, ard, 1:G.nM)

    for m in 1:G.nM
        sm_output[m] = sdp_benders_submf_bin(ss, δ, m, G);
        sm[m] = sm_output[m].sm;
    ...
    if flg_all == 0 # All subproblems were feasible
        ...
        cut_cte2 = 0.0;
        cut_coef2 = zeros(G.nT - 1, G.nM);

        # println("Generating OPTIMALITY Cuts!!!");

        for m in 1:G.nM
            # Output bounds
            v = sum(Sm[m].dual_nu_y1[i,t] * (G.y_min[m,i] - Sm[m].y[i,t]) for t in G.N1m[m]:G.N2m[m], i in 1:nY[1]);
            # v = max(0,v)
            cut_cte += v
            v = sum(Sm[m].dual_nu_y2[i,t] * (Sm[m].y[i,t] - G.y_max[m,i]) for t in G.N1m[m]:G.N2m[m], i in 1:nY[1]);
            # v = max(0,v)
            cut_cte += v

            # bounds on control variation
            v = sum(Sm[m].dual_nu_du1[i,t] * (G.Du_min[m,i] - Sm[m].Du[i,t]) for t in 1:G.Num[m] - 1, i in 1:nU[1]);
            # v = max(0,v)
            cut_cte += v
            v = sum(Sm[m].dual_nu_du2[i,t] * (Sm[m].Du[i,t] - G.Du_max[m,i]) for t in 1:G.Num[m] - 1, i in 1:nU[1]);
            # v = max(0,v)
            cut_cte += v
            ...
            v = sum(dot(Sm[m].dual_nu_u2[:,t], Sm[m].u[:,t]) - dot(Sm[m].dual_nu_u1[:,t], Sm[m].u[:,t]) for t in 1:G.Num[m] - 1, r in 1:G.nR);
            # v = sum((Sm[m].dual_nu_u2[j,t] * Sm[m].u[j,t]) - (Sm[m].dual_nu_u1[j,t] * Sm[m].u[j,t]) for t in 1:G.nT - 1, r in 1:G.nR, j in 1:nU[1]);
            # v = max(0,v)              
            cut_cte2 += v;    

            # for t in 1:G.Num[m] - 1           
            #     for j in 1:nU[1]
            #         cut_coef2[t,m] = ((Sm[m].dual_nu_u1[j,t] * G.u_min[m,j]) - (Sm[m].dual_nu_u2[j,t] * G.u_max[m,j]));
            #     end
            # end
            for t in 1:G.Num[m] - 1           
                cut_coef2[t,m] = (dot(Sm[m].dual_nu_u1[:,t], G.u_min[m,:]) - dot(Sm[m].dual_nu_u2[:,t], G.u_max[m,:]));
            end  
        end
    ...
    # tempo1 = @elapsed sm_output = pmap(sdp_benders_submi_bin, ar, ar3, ar4, ard, 1:G.nM)

    # for m in k
    for m in 1:G.nM
        sm_output[m] = sdp_benders_submi_bin(ss, 0, 0, δ, m, G);
        sm[m] = sm_output[m].sm;
    ...
    #                        Sm[m].dual_mu_Du_B[t,i] * ( -(Sm[m].Du[i,t] - (Sm[m].u[i,t] - Sm[m].u[i,t - 1])) ) for t in 2:G.nT - 1, i in 1:nU[1]);
                 
    #         cut_cte += sum(Sm[m].dual_mu_X_A[t,i] * (Sm[m].x[i,t] - (dot(Am[m,i,:], Sm[m].x[:,t - 1]) + dot(Bm[m,i,:], Sm[m].u[:,t - 1]))) + 
    #                        Sm[m].dual_mu_X_B[t,i] * - (Sm[m].x[i,t] - (dot(Am[m,i,:], Sm[m].x[:,t - 1]) + dot(Bm[m,i,:], Sm[m].u[:,t - 1]))) for t in 2:G.nT, i in 1:nX[1])

    #         cut_cte += sum(Sm[m].dual_mu_Y_A[t,i] * (Sm[m].y[i,t] - (dot(Cm[m,i,:], Sm[m].x[:,t]) + dot(Dm[m,i,:], Sm[m].u[:,t - 1]))) + 
    #                        Sm[m].dual_mu_Y_B[t,i] * - (Sm[m].y[i,t] - (dot(Cm[m,i,:], Sm[m].x[:,t]) + dot(Dm[m,i,:], Sm[m].u[:,t - 1]))) for t in 2:G.nT, i in 1:nY[1])
    #     end
    # end
